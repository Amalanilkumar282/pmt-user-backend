...
{
    public class GetIssueCountByStatusBySprintQueryHandler
        : IRequestHandler<GetIssueCountByStatusBySprintQuery, ApiResponse<Dictionary<string, int>>>
    {
        private readonly IIssueRepository _issueRepository;
        private readonly IStatusRepository _statusRepository;

        public GetIssueCountByStatusBySprintQueryHandler(
        ...
        public async Task<ApiResponse<Dictionary<string, int>>> Handle(
            GetIssueCountByStatusBySprintQuery request, 
            CancellationToken cancellationToken)
        {
            // Fetch all issues for the sprint
            var issues = await _issueRepository.FindAsync(i => i.SprintId == request.SprintId);
            ...
            return ApiResponse<Dictionary<string, int>>.Success(
                result,
                $"Successfully retrieved issue count by status for sprint.");
        }
    }
}